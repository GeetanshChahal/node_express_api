...
const UUID =require ('uuid-int');    //to get random id as int
...
router.post('/',(req,res)=>{    //for creating a new teacher
    ...
    const tchr_id=UUID(0).uuid();
    teachers.push({...tchr ,id:tchr_id});   
    res.send(`Teacher with the name ${tchr.name} is added.`);
});

router.get('/:id',(req,res) =>{     //to get a teacher info as per the given id 
    const {id}=req.params;
    const foundTeacher=teachers.find((teacher)=>teacher.id== id);
    res.send(foundTeacher)
});

router.delete('/:id',(req,res)=>{       //to delete a teacher info as per the given id
    const {id}=req.params;
    teachers =teachers.filter((tchr) => tchr.id!=id);   //which are not equal to the id given, we need to keep them in array,and delete other one which is matching
...
router.patch('/:id',(req,res)=>{    //to update the data of a teacher 
    ...
    if(name) tchr.name=name;
    if(department) tchr.department=department;
    if(age) tchr.age=age;
    if(street) tchr.address.street=street;
    if(city) tchr.address.city=city;
    if(state) tchr.address.state=state;
    if(pin) tchr.address.pin=pin;
